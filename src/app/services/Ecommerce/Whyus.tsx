...
const WorkWithUs = () => {
  return (
    <section className="flex flex-col items-center justify-center min-h-screen px-6 py-12">
      ...
      >
        Why Work with <span className="text-blue-400">Us?</span>
      </motion.h2>

      {/* Timeline Wrapper */}
      <div className="relative w-full max-w-4xl">
        {/* Vertical Line */}
        <div className="absolute left-1/2 transform -translate-x-1 bg-blue-500 w-1 h-full hidden md:block"></div>
        ...
        <div className="flex flex-col gap-12">
          {benefits.map((benefit, index) => (
            ...
            >
              {/* Circle Indicator */}
              <div className="hidden md:flex absolute left-1/2 transform -translate-x-1/2 w-5 h-5 bg-blue-500 rounded-full"></div>

              {/* Content Box */}
              <div
                className="w-full md:w-1/2 p-6 bg-gray-900 rounded-lg shadow-lg text-center md:text-left border border-gray-700 hover:border-blue-400 transition-all duration-300"
              >
                <h3 className="text-lg font-semibold text-blue-400">{benefit.title}</h3>
